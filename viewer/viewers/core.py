...
class ImageViewer(QtGui.QMainWindow):
    ...
    Parameters
    ----------
    image : array or Plugin
        Image being viewed.  If a Plugin is provided, then the
        output of the plugin is visualized.
    ...
    original_image_changed = Signal(tuple)
    ...
    def __init__(self, image):
        ...
        if isinstance(image, Plugin):
            ...
            plugin.output_changed.connect(self._update_original_image)
        ...
        self._update_original_image((image, None))
        self.plugins = []
    ...
    def _update_original_image(self, (image, data)):
        """
        `data` will only be present if the viewer is attached
        to the output of a plugin.
        """
        if image is None:
            image = self.image
        else:
        ...
        self.original_image_changed.emit((image, data))
